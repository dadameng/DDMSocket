...
/** 将数值转成字节。编码方式：低位在前，高位在后 */
+ (NSData *)byteFromFloat:(float)val;
+ (NSData *)bytesFromDouble:(int16_t)val;
+ (NSData *)bytesFromDouble:(int64_t)value byteCount:(int)byteCount;
+ (NSData *)bytesFromDouble:(int64_t)value byteCount:(int)byteCount reverse:(BOOL)reverse;
